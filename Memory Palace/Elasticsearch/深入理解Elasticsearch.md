...
![image-20220117134619944](./pics/Elastic_Stack.png)
...
从7.0开始，无需配置该参数。
7.0开始采用raft算法选主
https://www.easyice.cn/archives/332#1_Raft
...
![image-20220119142334588](./pics/Lucene_Index.png)
...
![image-20220119142358950](./pics/refresh.png)
...
![image-20220119143711981](./pics/Transaction_log.png)
...
![image-20220119145243163](./pics/flush.png)
...
​	merge操作比较重，一般非必须不建议手动执行。从hot节点移动到warm节点时，建议执行一次。
...
![image-20220119145243163](./pics/Query-phase.png)
...
![image-20220120172027968](./pics/fetch-phase.png)
...
![image-20220120153423311](./pics/relevance-score.png)
...
* DF ： 检索词在所有文档中出现的频率
  ...
  * “应用”在相对比较多的文档中出现
  * “的”在大量的文档中出现
* IDF （ Inverse Document Frequency ） = log（全部文档数/ 检索词出现的文档数）
  * 检索词出现的文档数越小，IDF越大，表示查出来的doc可能和查询语句的相关性越高
* TF-IDF本质上就是将TF 求和变成了**加权求和**
  * TF（区块链） * IDF（区块链） + TF（的） * IDF（的） + TF（应用） * IDF（应用）

![image-20220120155521747](./pics/IDF.png)



#### Lucence中的 TF-IDF 评分公式

![image-20220120162738404](./pics/TF-IDF_equation.png)



#### BM25

![image-20220120162912328](./pics/BM25.png)
...
![image-20220120165504793](./pics/bool_query.png)
...
![image-20220120172927986](./pics/what_is_agg.png)
...
![image-20220121144812554](./pics/bucket_metric_agg.png)

##### Bucket

![image-20220121144930455](./pics/bucket_agg.png)



##### Metric

![image-20220121144956734](./pics/metric_agg.png)
...
![image-20220125101733399](./pics/data_role.png)
...
![image-20220125105921263](./pics/basic_deploy.png)

#### 水平扩展

* 当系统中有大量负责聚合及聚合时，增加Coordinating节点，增加查询性能

![image-20220125110124648](./pics/load_balance_coordinate.png)



* 也可以将Coordinating节点划分为读写分离

![image-20220125110315578](./pics/read_write_sqp.png)

* kibana通常部署在coordinating节点

![image-20220125110354218](./pics/kibana_load_balance.png)
...
```shell
PUT traceIndex-2022-01-25/_settings
{
...
```
 ...
 ![image-20220125114206993](./pics/index_routing_allocation.png)
...
* 从ES 5 开始，支持Development 和 Production 两种运行模式
  ...
  ![image-20220125150443037](./pics/dev_mode_pro_mode.png)
